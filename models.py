...
HUAYAN_CODE_REASONING_MODEL = "code-generation-reasoning-latest"
...
def get_huayan_model_client(
    ...
    【使用场景】
    当你想使用公司内部私有化部署的大模型时使用这个函数。
    """
    return get_lc_model_client(
        api_key=os.getenv(HUAYAN_API_KEY_OS_VAR_NAME),
        base_url=HUAYAN_URL,
        model=model,
        temperature=temperature,
        verbose=verbose,
...
def get_huayan_embeddings(
    ...
    function_name = inspect.currentframe().f_code.co_name
    if verbose:
        print(f"【{function_name}】连接平台: {base_url}, 模型: {model}")
    if debug:
        print(f"【{function_name}】连接平台: {base_url}, 模型: {model}, Key: {api_key}")
...
class HuayanRerank:
    ...
    实现 LangChain 的 DocumentCompressor 接口，用于对检索结果重新排序。
    ...
    def __init__(self, model=HUAYAN_RERANK_MODEL, top_n=5):
        self.model = model
        self.top_n = top_n
        self.api_key = os.getenv(HUAYAN_API_KEY_OS_VAR_NAME)
